...
```
公众号 API ──► sync_articles.py ──► <web根>/articles.json + assets/insights/*.jpg
                                         │
...
```bash
# 1. 把仓库拉到服务器（只为拿 scripts/，与 /var/www/tuce 分开）
git clone <repo> /opt/tuce-liuxue-h5        # 或 git pull 更新

# 2. 配密钥
cd /opt/tuce-liuxue-h5
cp scripts/sync_articles.env.example scripts/.env
vi scripts/.env        # 填 WX_APPID / WX_APPSECRET；TUCE_WEB_ROOT=/var/www/tuce
chmod 600 scripts/.env

# 3. 手动跑一次验证（会写 /var/www/tuce/articles.json）
set -a; source scripts/.env; set +a
python3 scripts/sync_articles.py -v
curl -s http://127.0.0.1/articles.json | head     # 确认线上已更新

# 4. 配 cron：每天 03:30 同步一次（公众号更新不频繁，无需更密）
crontab -e
# 加入（注意用绝对路径；日志留档便于排查）：
30 3 * * * cd /opt/tuce-liuxue-h5 && env $(grep -v '^#' scripts/.env | xargs) /usr/bin/python3 scripts/sync_articles.py >> /var/log/tuce-sync.log 2>&1
```
...
为此 `deploy.sh` 已加 `--exclude`，保护这两项不被部署清掉：
